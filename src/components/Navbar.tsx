          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-4 py-2 text-[15px] text-white/80 font-semibold tracking-wide transition-colors duration-200 hover:text-white rounded-md hover:bg-white/[0.06]"
              >
                {link.label}
              </Link>
            ))}
